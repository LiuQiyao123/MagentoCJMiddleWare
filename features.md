...
-   **增强业务指标监控**: 当前监控仅覆盖HTTP技术指标。未来可增加更多业务层面的监控项，例如 `orders_synced_total` (区分成功/失败)、`products_mapped_total`、`cj_token_refresh_total` 等，以便更精准地掌握业务健康状况。
-   **Webhook 安全性**: 增加 Webhook 签名的验证机制，确保所有传入的请求都来自于受信任的 Magento 源。
-   **多店铺支持**: 完善 `Store` 模型的管理和使用，使系统能够真正支持同时对接多个不同的 Magento 店铺。
-   **自动化测试**: 引入单元测试和集成测试，覆盖核心的同步逻辑和 API 端点，保障代码质量和未来的重构安全。
